...
## Workflow
1. Decide whether the task introduced a durable change.
...
2. If there is no lasting change:
   - set `knowledge_review_status: not_needed`
   - set `knowledge_reviewed_at`
   - leave a short reason in the task status or follow-up fields
3. If durable change exists, map it to the correct targets.
...
8. When running a weekly review instead of a single-task closeout, explicitly check for:
...
2. The right long-term docs are updated.
3. `knowledge/changelog.md` records the durable update.
4. Any reusable workflow is either linked to an existing skill or handed off as a candidate.
