...
3. Reusable knowledge in `knowledge/`
4. A durable knowledge loop that turns repeated validated workflows into skill candidates instead of leaving them buried in task notes
...
4. Ensure task metadata supports durable-knowledge review:
   ...
   - `knowledge_followups`
   - `skill_candidates`
5. Maintain `docs/work/.current` pointer to the active task folder.
...
7. Verify `AGENTS.md` stays stable-rule only; progress belongs in `docs/work/.../status.md`.
8. Summarize deliverables, pending confirmations, and recommended next update trigger.
...
2. Add critical decisions to `decisions.md`
3. Update `meta.yaml.updated_at`

At task completion:
1. Set `knowledge_review_status` to one of `not_needed`, `updated`, or `followup_required`
2. Extract reusable conclusions into `knowledge/project/`
3. Link related knowledge ids in `meta.yaml.related_knowledge`
4. If the task surfaced a repeatable cross-project workflow, add or update a `skill_candidates` entry instead of leaving it only in task prose
