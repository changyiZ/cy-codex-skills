...
2. If repository is not locked, default recommendation is Godot `4.6.1-stable`.
3. Upgrade engine only when explicitly requested.

## Docs Policy

1. Use official stable Godot docs first.
2. Use latest docs only for unstable or unreleased behavior checks.
3. Use the class reference for exact API validation.
